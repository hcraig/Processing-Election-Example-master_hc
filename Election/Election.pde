...
void draw() {
  ...
  if (millis() - lastStateMillis >= MILLIS_PER_STATE) {
    ...
    fill(50,50,250);  // blue
    text("discrepency between Obama and Romney",WINDOW_SIZE/4,WINDOW_SIZE/2);
    text(Math.round(state.pctForObama - state.pctForRomney),WINDOW_SIZE/4,3*WINDOW_SIZE/4);
    // draw the romney vote count and title
    fill(201,50,50);  // red
//    text("Romney",3*WINDOW_SIZE/4,WINDOW_SIZE/2);
//    text(Math.round(state.pctForRomney)+"percent",3*WINDOW_SIZE/4,3*WINDOW_SIZE/4);
